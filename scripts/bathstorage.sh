...
if [[ -d "$PWD/$movie" ]]; then
	for f in "$PWD/$movie"/*
	do
		sudo scp -i ~/.ssh/github -P 3972 "$f" bathhouse@penwing.org:/home/bathhouse/downloads/
	done
    
elif [[ -f "$PWD/$movie" ]]; then
	sudo scp -i ~/.ssh/github -P 3972 "$PWD/$movie" bathhouse@penwing.org:/home/bathhouse/downloads/
else
